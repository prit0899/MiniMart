...
**Latest commit:** `ec75c39` — *owner playtest batch: clutter removal, cashier-on-purchase, phone-order UX, buyer wait+progress, hen/farm readouts* (earlier this session: iOS device-readiness build, MegaMart upgrade-panel wiring)
...
| 26 | **Owner playtest batch (2026-07-21)** — 9 concrete issues from actually playing | Play-only findings again: (a) green-circle player upgrade pads (Speed/Carry/Crop) were useless clutter; (b) Counter 2's cashier appeared by store LEVEL, so it staffed a till the player never bought; (c) a red-white striped gate by Counter 1 read as meaningless decoration; (d) phone-order card had a Collect button (instant-fulfil shortcut) + Close button and was always on screen; (e) buyers LEFT when a wanted item was out of stock instead of waiting; (f) no per-item shopping progress on buyers; (g) hen's tomato-in/egg-out was unreadable; (h) farms showed no growth progress. | ✅ Done & compile-verified (entered Play, 0 exceptions) across 3 commits (`3529f2e`, `6659140`, `ec75c39`): removed the upgrade pads (carry still upgrades in the panel — reference PLAYER row; reference has no player speed/crop upgrade) and the striped gate; **Counter 2 cashier is now hired BY PURCHASE** (`CashCounter` gates on its own active state, which the purchase pad sets; `OnEnable` refreshes so the cashier appears on buy, not at a level); **phone orders** fulfilled only by physically carrying items to the van (`PlayerInteraction` already loads on contact — Collect/Close buttons hidden; card shows ONLY when the player is near the van); **buyers now commit to a shelf and WAIT** for a restock up to `RestockWaitSeconds`=25s, resuming instantly on refill, leaving only if nothing was ever collected (keeps the anti-'statue' bound from row #17/#22); **thought bubble** shows item icon + have/want (4/5) + progress bar, advancing per item; **HenBadge** ('Tom N  Egg M') and **FarmBadge** (ripe/cap + fill bar on every farm). **RUNTIME-VERIFIED via computer-use screenshots** (MCP camera stayed revoked, so I drove the editor directly): opened `Game.unity`, entered Play → world builds, **`DataValidator ✅ ALL PASS 43,062/0`**, zero game-code exceptions, console 0 errors, FarmBadge renders with its green fill bar, store healthy with the pads/gate removed. Badges inherit the same `WorldBadge` sizing (characterSize 0.03) as the long-accepted machine/storage badges, so sizing is consistent. **Lesson:** several *earlier* chunk "verifications" this session had entered Play on a stray empty **Untitled** scene (no `_Bootstrap`), so they proved compilation only, not runtime — always confirm `Game.unity`/`MegaMart.unity` is the open scene (hierarchy shows `_Bootstrap`) before trusting a play-boot as runtime proof. |

| 27 | **Systematic reference-parity audit vs `Refer/` screenshots (2026-07-21)** | Went feature-by-feature through all reference gameplay + UI screenshots. **Parity is high:** Upgrades panel (Workers/Machines/Animals tabs, Stack+Speed, Maxed badges) matches exactly; two marts + travel, all farms (tomato/wheat/corn/hen/cow), all machines, shelves w/ count badges, thought bubbles (item + laptop-when-queuing), trolleys, tutorial arrow, settings/cash/joystick, worker roles — all present. **Deltas found:** (1) reference badges show a small **item icon beside the count** (🍎5/8); (2) a **smiley 😊 button**; (3) decorative red trash bins + a "Cafe Mart" labeled van; (4) the red-white striped barrier. | ✅ **RESOLVED — every delta closed or owner-decided:** (1) **DONE** — farm badges now show a crop-color icon chip + count + bar (`FarmBadge`, `3dc27a2`), verified live; (2) **DEFERRED by owner** — "we don't need it right now, note for future updates" → see Future Work below; (3) **DONE** — `PrimitiveFactory.TrashBin` places red bins in both marts (`0d8edae`); the "Cafe Mart" van label was already satisfied (travel pad reads "GO TO MEGAMART"); (4) the striped barrier IS in the reference but the **owner told us to remove it** (owner overrides parity). All implemented deltas runtime-verified (`DataValidator ALL PASS`, 0 game-code exceptions). |

| 28 | **External playtest bug batch (`Docs/Playtest-Report-2026-07-27.md`) + my own findings (2026-07-21 fixes)** | The report found: B1 no camera clamp (empty grass at edges), B2 store starves at L10 (buyers walk out), B3 panels stack, B4 pricing overflows/no-scroll, B5 blank buttons (missing icon sprites), B6 oversized floating egg, B7 translucent pricing panel, B8 hard-edged smoke squares, B9 NET button lingers; plus balance (worker/machine upgrades never required; offline $720 vs +3 goods) and legibility (white-text shelf badges, tiny joystick). | ✅ **Fixed & verified (`3403d9a`, DataValidator 42,762/0, 0 game-code exceptions, visually confirmed):** B1 `CameraFollow` bounds clamp (focus stays inside the building); B2 `BuyerSpawner.MinSpawnInterval` 1.5s floor so demand can't outrun a base-rate store; B3 `CloseAllCenterPanels()` single-panel rule; B7 shells opaque (1.0α); B8 smoke size-over-lifetime; B9 NET refresh moved before pause gate; shelf `StockBadge` → dark chip + item-color icon (legibility + reference parity); offline coins 0.3→0.18×/level; prices whole-dollar; joystick base more visible. ✅ **Follow-ups now also fixed (`586b92d`, validator 42,644/0):** B4 Pricing list wrapped in a ScrollRect (scrolls inside the panel); B5 close buttons always draw a white "X" over the glyph-less `btn_close` art + offer toggle labelled "%"; B6 the "floating egg" was the **production-source Locator icon** (`ItemMesh` scale 3.5 / height 3.4) — shrunk to 2.0/2.6 so it reads as a marker, not a glitch; worker upgrade **top costs raised** (Shelver→1000, Farmer→1800, Chef→3500) as a late-game cash sink (early ranks untouched). Visually confirmed: pricing scrolls/opaque with a visible X, whole-dollar prices, smaller locators. |

### Future Work (owner-deferred, not gaps)
- **Smiley 😊 button** (reference has one, left side under Upgrades). Owner on 2026-07-21: *"we don't need it right now. note down this for future updates."* When picked up, options discussed: a **Store Rating** panel (live 1–5 stars from `CustomersServed` vs a lost-customers count) or a **Customer Reviews** flavor feed.
